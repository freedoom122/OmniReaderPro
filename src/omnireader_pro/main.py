"""OmniReader Pro — application entry point.
...
from omnireader_pro import APP_NAME, __version__
...
def _bootstrap_services():
    ...
    from omnireader_pro.app.logging_setup import prune_old_logs, setup_logging
    ...
    paths.ensure_all()
    setup_logging(verbose=bool(os.environ.get("OMNIREADER_VERBOSE")))
    prune_old_logs()
...
class _SingleInstanceGuard:
    ...
    def __init__(self) -> None:
        ...
        self._server = None
        self._socket = None
        self.server_name = f"omnireader-pro-{__import__('getpass').getuser()}"
...
def _install_exception_hook() -> None:
    def hook(exc_type, exc_value, exc_tb):
        try:
            from omnireader_pro.app.logging_setup import logger  # noqa
            import logging
            logging.getLogger("omnireader").error(
...
def run(argv: list | None = None) -> int:
    ...
    app.setOrganizationName("OmniReader")
    ...
    from omnireader_pro.app import paths
    from omnireader_pro.app.logging_setup import setup_logging
    from omnireader_pro.ui.icons import make_window_icon
    app.setWindowIcon(make_window_icon())

    settings, db, tasks, journal = _bootstrap_services()

    # Smoke-test mode: OMNIREADER_SMOKE=1 quits cleanly after a short run,
    # used by CI and by build validation to prove the app boots and exits.
    smoke_seconds = os.environ.get("OMNIREADER_SMOKE", "")
    if smoke_seconds:
    ...
    if settings.restored_defaults:
        from omnireader_pro.ui.widgets import show_toast
        show_toast(window,
    ...
    try:
        ...
        import logging
        logging.getLogger("omnireader").exception("session restore failed")

    # Command-line files (double-click associations / drag onto exe).
    for arg in sys.argv[1:]:
        if arg.lower().startswith("omnireader-smoke-test"):
            continue
    ...
    try:
        from omnireader_pro.utils.safeio import cleanup_temps
