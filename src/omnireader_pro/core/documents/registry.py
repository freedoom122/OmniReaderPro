...
from omnireader_pro.utils.pathutils import SUPPORTED_EXTENSIONS

logger = logging.getLogger("omnireader.registry")
...
def open_document(path: Path, password: str = "") -> OpenResult:
    ...
    try:
        ...
        if ext in MOBI_EXTS:
            from omnireader_pro.core.documents.mobi_engine import MobiEngine
        ...
        if ext in COMIC_EXTS:
            ...
            from omnireader_pro.core.documents.comic_engine import CbrEngine
        ...
        if ext in TEXT_EXTS:
            from omnireader_pro.core.documents.text_engine import TextEngine
            return OpenResult(TextEngine(path), format_name="Text")
        if ext in HTML_EXTS:
            from omnireader_pro.core.documents.html_engine import HtmlEngine
        ...
        try:
            ...
            pass
        return OpenResult(
            error=f"OmniReader cannot open {ext.upper() or 'this'} files. "
