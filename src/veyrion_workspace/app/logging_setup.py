...
logger = logging.getLogger("veyrion")
...
def _setup_file_handler(log_dir: Path, verbose: bool) -> logging.Handler:
    ...
    handler = logging.handlers.RotatingFileHandler(
        log_dir / "veyrion.log", maxBytes=1_500_000, backupCount=4, encoding="utf-8"
    )
...
def setup_logging(verbose: bool = False) -> Path:
    """Initialize the root 'veyrion' logger. Returns the log directory."""
    log_dir = paths.logs_dir()
    log_dir.mkdir(parents=True, exist_ok=True)

    root = logging.getLogger("veyrion")
...
def export_diagnostic_report(destination: Path) -> Path:
    ...
    def safe_sysinfo() -> str:
        lines = [
            "Veyrion Workspace Diagnostic Report",
    ...
    with gzip.open(destination, "wt", encoding="utf-8") as fh:
        ...
        log_file = paths.logs_dir() / "veyrion.log"
        if log_file.exists():
            try:
...
def prune_old_logs(keep_days: int = 30) -> None:
    ...
    if not log_dir.exists():
        return
    for f in log_dir.glob("veyrion.log*"):
        try:
