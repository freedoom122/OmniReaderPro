...
logger = logging.getLogger("veyrion.signing")
...
def sign_pdf(source: Path, target: Path, identity: SigningIdentity,
    ...
    if not HAS_PYHANKO:
        raise RuntimeError(
            ...
            "(pip install pyhanko). Veyrion does not fake signatures by "
            "stamping images.")
    ...
    pdf_signer = signers.PdfSigner(
        signers.PdfSignatureMetadata(field_name="Veyrion-Signature",
