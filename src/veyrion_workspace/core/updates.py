...
from veyrion_workspace import __version__

logger = logging.getLogger("veyrion.updates")

UPDATE_MANIFEST_URL = "https://api.github.com/repos/veyrion-pro/veyrion-pro/releases/latest"
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={"Accept": "application/vnd.github+json",
                     "User-Agent": f"VeyrionWorkspace/{__version__}"})
...
def run_update_check_dialog(parent) -> None:
    ...
    if result.get("error"):
        QMessageBox.information(
            parent, "Update check",
            "Could not reach the update server. Veyrion works fully "
    ...
    if result["available"]:
        ...
        box.setInformativeText(
            "Veyrion never downloads or installs updates on its own. "
